...
class FamilyDetailScreen extends ConsumerWidget {
  ...
  void _showInviteByEmailDialog(
    ...
    FamilyMemberRole selectedRole = FamilyMemberRole.member;
...
class _FamilyDetailBody extends ConsumerWidget {
  final FamilyWithMembers familyData;
  ...
  Future<void> _removeMember(
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('¿Eliminar a ${member.displayName ?? member.userEmail}?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('Cancelar'),
...
class _MemberTile extends StatelessWidget {
  ...
  final Function(FamilyMemberRole) onChangeRole;
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Row(
          children: [
            Text(member.displayName ?? member.userEmail ?? 'Usuario'),
            if (isCurrentUser)
              const Padding(
            ...
            ? PopupMenuButton<String>(
                onSelected: (value) {
                  if (value == 'remove') {
                    ...
                    onChangeRole(FamilyMemberRole.values
