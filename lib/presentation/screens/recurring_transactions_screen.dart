...
class _RecurringTile extends ConsumerWidget {
  final RecurringTransactionEntry item;
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Text(
                  'Próximo: ${dateFormat.format(item.nextExecutionDate)}',
                  style: theme.textTheme.bodySmall?.copyWith(
                    color: theme.colorScheme.primary,
...
/// Formulario para crear transacción recurrente
...
class _RecurringFormSheetState extends ConsumerState<_RecurringFormSheet> {
  ...
  Future<void> _save() async {
    ...
    try {
      // Necesitamos una categoría por defecto - usar la primera disponible
      // TODO: Agregar selector de categoría al formulario
      const defaultCategoryId = 'cat-gastos-servicios';

      await ref.read(recurringTransactionsNotifierProvider.notifier).create(
