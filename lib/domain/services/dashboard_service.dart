...
class DashboardService {
  ...
  _AssetLiabilityTotals _calculateAssetLiabilityTotals({
    ...
    final liabilityCategoryIds =
        categories.where((c) => c.type == 'liability').map((c) => c.id).toSet();
  ...
  _MonthlyTotals _calculateMonthlyTotals(List<TransactionEntry> transactions) {
  ...
  List<CategoryExpense> _buildExpensesByCategory({
    ...
    for (final entry in expenseMap.entries) {
      final category = categories.firstWhere(
        (c) => c.id == entry.key,
        orElse: () => _createUnknownCategory(entry.key, now),
      );
    ...
    for (final budget in budgets) {
      ...
      if (status != IndicatorStatus.good) {
        final category = categories.firstWhere(
          ...
          orElse: () => _createUnknownCategory(budget.categoryId, now),
        );
  ...
  Set<String> _findAllDescendants(
      String parentId, List<CategoryEntry> categories) {
  ...
  CategoryEntry _createUnknownCategory(String id, DateTime now) {
